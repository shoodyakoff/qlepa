...
export function formatStartText(onboardingText: string, checks: readonly DoctorCheck[]): string {
  ...
  const statusLines = hasFailures
    ...
    : [
        ...
        "В базовой настройке нет FAIL. WARN по референсам нормален, пока вы ещё не добавили фото или явно не написали: без моего фото.",
        `Фото-обложку по настроению не выбираем: в MVP она только ${PHOTO_COVER_PRESET}.`,
        "",
        "Следующий шаг",
        "Если хотите начать работу, напишите в чат: старт",
        "/start, start, старт — один и тот же сценарий Qlepa onboarding.",
        "Сначала пришлите недостающие данные бренда и референсы: название, ник, ТГ-канал, темы, аудиторию, тон, фото лица, фото по пояс или в полный рост — или напишите: без моего фото. Это без выбора настроения обложки.",
        "Локальный chrome сохраняется в private/brand.json, приватный голос — в brand/voice.local.md.",
        "Если бренд уже настроен, выберите режим первого поста: Editorial / Artifact-Rich или Series Digest.",
        "Для старого режима используйте posts/starter-post/post.md, для нового Series Digest — posts/starter-digest/post.md.",
